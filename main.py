...
def get_count_items_into_intermediary(items_name,df,list_itemsname):
    total = 0
    df_parcours = df.loc[df["name"] == f"{items_name}",["name","into"]]
    #print(df_parcours["name"])
        ...
        for list_items in df_parcours["into"]:
            for items in list_items:
                if(not(int(items) in df.index)):
                        total += 1
                        continue
                if(not(df.loc[int(items),["name"]][0] in list_itemsname)):
                    list_itemsname.append(df.loc[int(items),["name"]][0])
                    total+=get_count_items_into_intermediary(df.loc[int(items),"name"],df,list_itemsname)
            
            if(not(items_name in list_itemsname)):
                list_itemsname.append(items_name)
            total += 1
    else:
        return 1
...
def get_count_into_items(items_name,df):
    ...
    print(df_parcours)
    if df_parcours["into"].notna().any():
        total+=get_count_items_into_intermediary(items_name,df,list_itemsname)
    else:
        return 0
    list_itemsname.pop()
    df.at[df_parcours.index[0],"listitemsname"] = list_itemsname
    return total -1
...
def treating_data_items(version):
    ...
    df.to_excel(f'data_changes/items_{version}_v2.xlsx',index = True)
